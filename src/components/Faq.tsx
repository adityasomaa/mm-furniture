...
export function Faq({ locale }: { locale: Locale }) {
  return (
    <div id="faq" className="grid gap-10 lg:grid-cols-[minmax(0,22rem)_1fr] lg:gap-20">
      <div>
        <Kicker>FAQ</Kicker>
        <h2 className="mt-5 text-title text-espresso">{faqTitle[locale]}</h2>
      </div>

      <div className="border-t border-linen">
        {faq.map((f) => (
          <details key={f.q[locale]} className="group border-b border-linen">
            <summary className="flex cursor-pointer list-none items-start justify-between gap-6 py-5 text-head font-semibold text-espresso transition-colors duration-200 hover:text-brand [&::-webkit-details-marker]:hidden">
              <h3 className="text-head font-semibold">{f.q[locale]}</h3>
              <span
                aria-hidden="true"
                className="mt-1 shrink-0 text-brand transition-transform duration-300 ease-[cubic-bezier(0.165,0.84,0.44,1)] group-open:rotate-45"
              >
            ...
            <p className="prose-body pb-6 pr-8 text-[0.98rem] leading-relaxed text-clay">{f.a[locale]}</p>
          </details>
        ))}
      </div>
    </div>
